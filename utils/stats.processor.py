...
class StatsProcessor:
    ...
    def _store_player_stats(self, cursor, individual_stats: List[PlayerStat], matching_game, league_id: int, season: int, week: int):
        ...
        try:
            for player_stat in individual_stats:
                ...
                # Check if stat record already exists for this player in this game
                cursor.execute("""
                    SELECT * FROM player_stats 
                    WHERE player_id = ? AND game_id = ?
                """, (player_id, matching_game['game_id']))
                
                existing_stat = cursor.fetchone()
                
                if existing_stat:
                    # Update existing stat (additive - multiple categories per player)
                    self._update_player_stats_DEBUG(cursor, existing_stat['stat_id'],
                                            player_stat.stat_category, player_stat.stats, existing_stat)
                else:
    ...
    # Just the _update_player_stats method with debug output
    def _update_player_stats_DEBUG(cursor, stat_id: int, category: str, stats: Dict, existing_record):
        """Update existing player stats record (additive) - DEBUG VERSION"""
        print(f"\n[DEBUG] ==========================================")
        print(f"[DEBUG] _update_player_stats called")
        print(f"[DEBUG] stat_id: {stat_id}")
        print(f"[DEBUG] category: {category}")
        print(f"[DEBUG] stats: {stats}")
        print(f"[DEBUG] existing_record type: {type(existing_record)}")
        print(f"[DEBUG] ==========================================\n")
        
        # Convert existing_record to dict - always use dict() to be safe
        if isinstance(existing_record, dict):
            existing_dict = existing_record
            print(f"[DEBUG] existing_record was already a dict")
        else:
            existing_dict = dict(existing_record)
            print(f"[DEBUG] converted existing_record to dict")
        
        print(f"[DEBUG] existing_dict type: {type(existing_dict)}")
        ...
        print(f"[DEBUG] category (lowercase): {category}")
        print(f"[DEBUG] About to enter if/elif blocks")
        
        if category == "rushers":
            print(f"[DEBUG] In rushers block")
            ...
            print(f"[DEBUG] In passing block")
            print(f"[DEBUG] About to access existing_dict['passing_attempts']")
            ...
            print(f"[DEBUG] In receivers block")
            ...
            print(f"[DEBUG] In interceptions block")
            ...
            print(f"[DEBUG] In sacks block")
            updates['sacks'] = existing_dict['sacks'] + stats.get('sacks', 0)
            
        elif category == "kicking":
            print(f"[DEBUG] In kicking block")
            ...
            print(f"[DEBUG] In punt returns block")
            ...
            print(f"[DEBUG] In kickoff returns block")
    ...
    print("=" * 60)
    print("DEBUG VERSION OF _update_player_stats")
    print("=" * 60)
    print("\nReplace your _update_player_stats method in stats_processor.py")
    print("with the _update_player_stats_DEBUG function shown above.")
    print("\nThis will show exactly where the variable is being lost.")
    print("=" * 60) 
